...
TEST_F(test, le_strip_single_band)
{
    ...
    {
        bool ok = true;
        const auto *tag = tiff->tag(libertiff::Tag::StripOffsets);
...
TEST_F(test, tiled)
{
    ...
    {
        bool ok = true;
        const auto *tag = tiff->tag(libertiff::Tag::TileOffsets);
        ASSERT_NE(tag, nullptr);
    ...
    {
        bool ok = true;
        const auto *tag = tiff->tag(libertiff::Tag::TileByteCounts);
        ASSERT_NE(tag, nullptr);
...
TEST_F(test, image_description)
{
    ...
    auto tiff = libertiff::open(std::make_shared<libertiff::CFileReader>(f));
    ASSERT_NE(tiff, nullptr);
    const auto *tag = tiff->tag(libertiff::Tag::ImageDescription);
    ASSERT_NE(tag, nullptr);
    ASSERT_EQ(tag->type, static_cast<uint16_t>(libertiff::Type::ASCII));
    bool ok = true;
